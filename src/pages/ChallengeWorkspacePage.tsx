...
import { ChallengeDetail, SubmissionRecord, RunResult, ChallengeSummary } from '../types';
import { api } from '../api';
...
export const ChallengeWorkspacePage: React.FC = () => {
  ...
  const [activeLeftTab, setActiveLeftTab] = useState<'desc' | 'submissions' | 'hints'>('desc');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (challenge) {
        setChallenge({ ...challenge, status: result.status === 'VALID' ? 'VALID' : 'FAILED' });
      }
  ...
  if (!challenge) {
    return (
      <div className="max-w-md mx-auto my-20 p-6 bg-dark-900 border border-slate-800 rounded-xl text-center">
        ...
        <h2 className="text-lg font-bold text-white mb-1">Challenge Not Found</h2>
        <p className="text-xs text-slate-400 mb-6">The requested challenge &ldquo;{id}&rdquo; could not be found.</p>
        <Link
          to="/challenges"
      ...
      <div className="h-12 bg-dark-900 border-b border-slate-800/90 px-4 flex items-center justify-between gap-4 shrink-0 text-xs">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex items-center gap-1 text-slate-400">
            <button
              onClick={() => prevChallenge && navigate(`/challenges/${prevChallenge.id}`)}
              ...
              title={prevChallenge ? `Previous: ${prevChallenge.title}` : 'No previous challenge'}
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button
              onClick={() => nextChallenge && navigate(`/challenges/${nextChallenge.id}`)}
              disabled={!nextChallenge}
              className="p-1 rounded hover:bg-slate-800 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              title={nextChallenge ? `Next: ${nextChallenge.title}` : 'No next challenge'}
            >
          ...
          <div className="flex items-center gap-2 truncate">
            <span className="font-bold text-sm text-slate-100 truncate">{challenge.title}</span>
              ...
              <div className="p-4 space-y-3">
                ...
                {submissions.length === 0 ? (
                  <div className="py-12 text-center text-slate-500 text-xs">
                    ...
                    <p className="text-slate-600 mt-1">Click &ldquo;Submit&rdquo; to test your solution and record your verdict.</p>
                  </div>
                ) : (
            ...
            {activeLeftTab === 'hints' && (
              <div className="p-6 text-sm text-slate-300 space-y-4">
                ...
                <ul className="list-disc pl-5 space-y-2 text-slate-300 text-xs leading-relaxed">
                  <li>
                    <strong>Class & Method Names:</strong> Ensure your class is named <code className="text-brand-400 bg-slate-800 px-1 py-0.5 rounded">{challenge.className}</code> and methods have the exact signatures specified in the instructions.
                  </li>
                  <li>
                    <strong>Visibility:</strong> Mark your class as <code className="text-brand-400 bg-slate-800 px-1 py-0.5 rounded">public</code> so JUnit test runners can access and instantiate it.
                  </li>
                  <li>
                    <strong>Edge Cases:</strong> Consider boundary conditions like negative numbers, empty strings, null inputs, and zero values.
                  </li>
                  <li>
                    <strong>Execution Sandbox:</strong> Code is compiled with <code className="text-brand-400 bg-slate-800 px-1 py-0.5 rounded">javac</code> and tested using JUnit 5 in an isolated sandbox with a 10s execution timeout.
                  </li>
                </ul>
              </div>
            )}
